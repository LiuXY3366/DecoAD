...
from UNVAD.stage1.dataset import get_cluster_dataset
from UNVAD.stage1.fusion import Autoencoder
from tqdm import tqdm

from UNVAD.KG.knowledge_graph import create_relation
...
def scene_cluster(auc_1 = 0.,flag1 = 0):
    # 指定要保存的文件名
    file_name = f"{args.UNVAD}KG/data/cluster/scene_cluster_centers.txt"
    # 检查文件是否存在
    if os.path.exists(file_name):
        os.remove(file_name)
    if not os.path.exists(file_name):
        ...
        if args.dataset == 'UBnormal':
            for i in range(29):
                data = torch.load(f"{args.data_dir}{args.dataset}_scene_feature/scene{i + 1}_features.pth")
                dataset_na += data
            ...
            for i in range(len(scene_arr)):
                data = torch.load(f"{args.data_dir}{args.dataset}_scene_feature/scene{scene_arr[i+1]}_features.pth")
                dataset_na += data
        else:
