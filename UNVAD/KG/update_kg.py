...
import shutil
import torch
from UNVAD.KG.knowledge_graph import search_all_relation, create_relation
import re
import numpy as np
from scipy.spatial import distance
from tqdm import tqdm
from UNVAD.KG.cluster import get_cluster
from UNVAD.KG.knowledge_graph import clean_all, init_anything, clean_no_relation
...
def getNodeAndRela(file_name = f'{args.UNVAD}KG/data/cluster/main_relation.txt'):
...
def make_scene(dataset,file_name = f'{args.UNVAD}KG/data/cluster/main_scenes.txt'):
    if os.path.exists(file_name):
        os.remove(file_name)
    # 这里还需要修改一下
    for i in range(13):
        # 加载 .pth 文件
        feature_path = f"{args.DATA}{dataset}_scene_feature/scene{i+1}_features.pth"
...
def make_pose_dict(filename = f'{args.UNVAD}KG/data/cluster/main_poses.txt'):
    # 初始化一个空字典
    data_dict = {}
...
def make_scene_dict(filename = f'{args.UNVAD}KG/data/cluster/main_scenes.txt'):
...
def make_relation_arrs(filename = f'{args.UNVAD}KG/data/cluster/main_relation.txt'):
    # 初始化三个空数组
    relation_arr = []
    scene_arr = []
    pose_arr = []
...
def make_sub_kg(construct = False,auc_1 = 0.78500,flag1 = 1):
    if construct:
        ...
        clean_no_relation()
        print('sub图构建完成！')
        source_file = f'{args.UNVAD}KG/data/cluster/cluster_centers.txt'
        destination_file = f'{args.UNVAD}KG/data/cluster/sub_poses.txt'
        shutil.copy(source_file, destination_file)
        getNodeAndRela(f'{args.UNVAD}KG/data/cluster/sub_relation.txt')
        make_scene(f'{args.dataset}',f'{args.UNVAD}KG/data/cluster/sub_scenes.txt')

# add == True:向所有main_txt添加新的数据
def add_data(add = False):
    main_poses_txt = f'{args.UNVAD}KG/data/cluster/main_poses.txt'
    sub_poses_txt = f'{args.UNVAD}KG/data/cluster/sub_poses.txt'
    main_scenes_txt = f'{args.UNVAD}KG/data/cluster/main_scenes.txt'
    sub_scenes_txt = f'{args.UNVAD}KG/data/cluster/sub_scenes.txt'
    main_relation_txt = f'{args.UNVAD}KG/data/cluster/main_relation.txt'
    sub_relation_txt = f'{args.UNVAD}KG/data/cluster/sub_relation.txt'
    step_relation_file = f'{args.UNVAD}KG/data/cluster/step_relation.txt'
...
def update_main_kg(update = False):
    file_path = f'{args.UNVAD}KG/data/cluster/main_relation.txt'
    if update:
        clean_all()
        init_anything(scene=400, pose=600)
