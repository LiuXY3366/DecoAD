...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(128*24+4+512, 256)  # 128是LSTM的隐藏层大小

    def forward(self, pose,path = '',scene_feature = ''):
        pose = self.getpose(pose)
        if scene_feature != '':
            pfe = self.pfe(path).unsqueeze(1)  # torch.Size([256, 20])
            # 处理pose特征
            pose_feature = pose.reshape(-1,128*24).unsqueeze(1)
            # 连接位置和scene
            scene_feature = torch.cat((pfe, scene_feature), dim=2)
            # 连接pose和scene特征
...
class Autoencoder(nn.Module):
    ...
    def forward(self, pose, path='' ,scene_feature=''):
        ...
        encoded_feature = self.encoder(pose,path, scene_feature)
