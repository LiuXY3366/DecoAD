...
from WSVAD.stage1.dataset import get_cluster_dataset
from WSVAD.stage1.fusion import Model
from tqdm import tqdm

from WSVAD.KG.knowledge_graph import create_relation
...
def scene_cluster(auc_1 = 0.,flag1 = 0):
    # 指定要保存的文件名
    file_name = f"{args.WSVAD}KG/data/cluster/scene_cluster_centers.txt"
    # 检查文件是否存在
    if os.path.exists(file_name):
        os.remove(file_name)
    if not os.path.exists(file_name):
