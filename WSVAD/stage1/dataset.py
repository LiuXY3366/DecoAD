...
def get_ab_labels_fully(global_data_np_ab, segs_meta_ab, path_to_vid_dir='', segs_root=''):
    ...
    for clip in tqdm(clip_list):
        ...
        gt_file = os.path.join(f"{args.data_dir}{dataset_name}/gt", clip.replace("annotations", "tracks.txt"))
...
class PoseSegDataset(Dataset):
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
        ...
        if abnormal_train_path is not None:
            ...
            elif args.supervise == 'S':
                print("全监督")
                ab_labels = get_ab_labels_fully(self.segs_data_np_ab, self.segs_meta_ab, path_to_vid_dir, abnormal_train_path)
            else:
                ab_labels = get_ab_labels(self.segs_data_np_ab, self.segs_meta_ab, path_to_vid_dir, abnormal_train_path)
                print("无监督?!")
            # print('ab_labels')
...
def gen_fusion_dataset_dataloader():
    ...
    for train in dataset_train:
        ...
        if args.dataset == 'UFSR':
            ...
            scene = torch.load(f"{args.data_dir}{args.dataset}_scene_feature/{mate[0]}{FN}/scene{scene}_features.pth")
        else:
            scene = mate[0]
            scene = torch.load(
