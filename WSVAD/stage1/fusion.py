...
class GetPose(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv1d(in_channels=args.seg_len, out_channels=1, kernel_size=3, padding=1)

    def forward(self, pose):
        pose = pose.permute(0, 2, 1, 3)
        pose = self.gcn(preprocess_data(pose), self.adj_matrix)
        pose = pose.reshape(-1,args.seg_len,36)
        pose = self.lstm(pose)
...
def main():
    ...
    skeleton_feature = torch.Tensor(np.random.rand(128,2,args.seg_len,18))
    scene_feature = torch.Tensor(np.random.rand(128,1, 512))
    path_feature = torch.Tensor(np.random.rand(128, args.seg_len, 2))
